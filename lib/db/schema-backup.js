...
export const boostEvents = pgTable('boost_events', {
    id: uuid('id').primaryKey().defaultRandom(),
    tokenBoostId: uuid('token_boost_id')
...
export const boostedTokenInfo = pgTable('boosted_token_info', {
    id: uuid('id').primaryKey().defaultRandom(),
    boostEventId: uuid('boost_event_id')
...
export const compositions = pgTable('compositions', {
    id: uuid('uuid_id').primaryKey(), // Use uuid_id column as the primary interface
    legacyId: serial('id'), // Keep old integer id as legacy
    userId: uuid('uuid_user_id')
        .notNull()
        .references(() => users.id),
