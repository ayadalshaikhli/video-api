...
export const whisperAudio = async (audioUrl, saveToFile = false) => {
  try {
    ...
    if (!cfAccountId || !cfApiToken) {
      consola.error(
        "❌ Cloudflare API credentials are not set in environment variables!"
      );
      throw new Error("Cloudflare API credentials not configured");
    }

    // Fetch the audio file from the provided URL (as binary data).
    ...
    const transcriptionResponse = await retryFunction(async () => {
      consola.info(
        "🚀 Sending audio to Cloudflare Whisper API for transcription..."
      );
